...
import { pipe, compose, find, flip, propEq, uniq, pluck, difference, filter, includes } from 'rambda';
...
async function app(state: State, list: List, dispatch: Dispatcher) {
  ...
  const stop = dispatch(async (_e: Event) => {
    ...
    const countryData = deserialise.popData(await io.fetchCountry(selectedCountryId))
    const newCountry = { ...selectedCountry, data: countryData } as FetchedCountry

    const newState = uniq([...state, newCountry]);

    const remainingCountries = difference(pluck('id', list), pluck('id', newState))
